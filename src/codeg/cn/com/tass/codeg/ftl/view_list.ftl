...
<html lang="en">
	...
	<body>
											...
											<div role="content">
												...
												<div class="row-fluid">
													<div class="span11 tableMenu">
														<div class="btn-group">
															<a class="btn btn-small" href="javascript:addObject()"><i class="cus-add"></i> <spring:message code='common.table.add' /> </a>
		...
		<!--  弹出ADD窗口 DIV -->
		<div id="${e_ClassName?uncap_first}AddDiv" class="hide popup form-horizontal">
			<form id="${e_ClassName?uncap_first}AddForm" action="${e_ClassName?uncap_first}/add" method="post">
				<#list e_Tableinfo.columns as column>
				<div class="control-group">
					<div class="row-fluid">
						<div class="span12">
							<label class="control-label pull-right"><i class="cus-sport-tennis"></i><spring:message code="${e_ModuleName}.${e_ClassName?uncap_first}.${column.propertyName}" /></label>
							<div class="controls">
								<#if column.propertyType=='Date'>
								<input type="text" class="focused span11 list-search-time"  rel='qTip'  readonly="true" name="${column.propertyName}" />
								<#else>
								<input type="text" title="<spring:message code='common.valid.name'/>" valid="required checkName" rel="qTip" class="span11 focused" name="${column.propertyName}" />
								</#if>
							</div>
						</div>
					</div>
		...
		<!--  弹出Edit窗口 DIV -->
		<div id="${e_ClassName?uncap_first}EditDiv" class="hide popup form-horizontal">
			<form id="${e_ClassName?uncap_first}EditForm" action="${e_ClassName?uncap_first}/edit" method="post">
				<input type="hidden" id="${e_ClassName?uncap_first}ID" name="${e_ClassName?uncap_first}ID" value="0" />
				<#list e_Tableinfo.columns as column>
				<div class="control-group">
					<div class="row-fluid">
						<div class="span12">
							...
							<div class="controls">
								<#if column.propertyType=='Date'>
								<input type="text" class="focused span11 list-search-time"  rel='qTip'  readonly="true" id="${column.propertyName}" name="${column.propertyName}" />
								<#else>
								<input type="text" title="<spring:message code='common.valid.name'/>" valid="required" rel="qTip" class="span11 focused" id="${column.propertyName}" name="${column.propertyName}" />
								</#if>
							</div>
						</div>
					</div>
			...
			oTable = $("#tableList").dataTable({
					 ...
					 "aoColumns" : [ {
							...
							"mData" : function(data,type,row){
								...
								var modifyIcon = "<a href='javascript:editObject("+data.${e_ClassName?uncap_first}ID+")' rel='tooltip' data-placement='left' title='<spring:message code='common.table.edit'/>'><i class='cus-pencil'></i></a>&nbsp;";
		...
		function addObject() {
				...
				$("#${e_ClassName?uncap_first}AddDiv").dialog(
								{
									title : "<spring:message code='${e_ModuleName}.${e_ClassName?uncap_first}.add'/>",
									autoOpen : true,
									width : 700,
									...
									buttons : {
										"<spring:message code="common.form.save"/>" : function() {
										   var result = validForm("${e_ClassName?uncap_first}AddForm");
										   if (result) {
												$("#${e_ClassName?uncap_first}AddForm").submit();
											}
										},
				   ...
				   initBlurValid("${e_ClassName?uncap_first}AddForm");
				   $("input,textarea","#${e_ClassName?uncap_first}AddForm").each(function(){$(this).val("");}); 
				
				   $('#${e_ClassName?uncap_first}AddForm').form({
						success : function(data) {
							$("#${e_ClassName?uncap_first}AddDiv").dialog("close");
